...
class ViewController: UIViewController {
    ...
    var profileList: [profile] = [
        profile(
            UUID: "1", profileName: "avt1", nameAndAge: "Anh Huân, 26",
            location: "Đang ở Thành phố Hồ Chí Minh", graduation: "Bằng cao đẳng/đại học",
            homeTown: "Quê quán Dak Lak", spiritual: "Đạo Phật"),
        profile(
            UUID: "2", profileName: "avt2", nameAndAge: "Cuong, 24",
            location: "Đang ở Bình Dương", graduation: "Bằng cao học",
            homeTown: "Quê quán Bình Dương", spiritual: "Vô thần"),
    ]
            ...
            if abs(howFar) < (view.frame.width - 200) {
                ...
                if howFar > 0 {
                    UIView.animate(
                        withDuration: 0.2,
                        animations: {
                            profile.center = CGPoint(
                                x: profile.center.x + 200, y: profile.center.y)
                            profile.transform = CGAffineTransform(rotationAngle: 0)
                        })
                } else {
                    UIView.animate(
            ...
            if let err = err {
                ...
                if let array = querySnapshot.value as? [[String: Any]] {
                    for aValue in array {
                        ...
                        let _p = profile(
                            profileName: profileName, nameAndAge: nameAndAge,
                            location: location, graduation: graduation,
                            homeTown: hometown, spiritual: spiritual)
